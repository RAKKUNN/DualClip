...
3. **Atomic Paste**는 시스템 클립보드를 잠시 교체하고, ⌘V를 시뮬레이션한 뒤, 원래 클립보드를 복원합니다 — 보통 약 200ms 이내이며 **설정 > General**에서 조절 가능

## 요구 사항

- macOS 13.0 (Ventura) 이상
- Intel 또는 Apple Silicon Mac — 사전 빌드된 릴리스는 유니버설 바이너리(`arm64` + `x86_64`)입니다
...
2. `DualClip-x.x.x-universal.zip` 다운로드
...
```
DualClipCore/                # 라이브러리 타겟 — 모든 앱 로직 (테스트 대상)
...
Tests/DualClipCoreTests/    # 유닛 테스트 (@testable import DualClipCore)
```
...
- [x] 정상 종료 시 RAM 0으로 채우기 (강제 종료 시에는 동작하지 않음)
